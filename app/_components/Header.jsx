...
export default function Header() {
  ...
  return (
    ...
    >
      <div className="h-full flex items-center justify-between">
        ...
        <nav className="hidden md:flex md:gap-4 lg:gap-8 items-center text-md font-medium text-gray-800">
          {navLinks.map((link) => (
            ...
            >
              {link.name}
              <span className="absolute left-0 -bottom-1 w-0 h-[2px] bg-[var(--primary-orange)] group-hover:w-full transition-all duration-300"></span>
            </Link>
          ))}
        </nav>
        ...
        <div className="hidden md:block">
          <Link href="/contact">
            <Button className="flex flex-row hover:gap-4 text-lg   transition-all ease-in-out bg-[var(--third-blue)] px-6 py-6 hover:bg-[var(--sec-blue)]">
              Contact Us
              <span className="">
                <FaArrowRightLong />
              </span>
      ...
      {/* Mobile Nav */}
      {isMenuOpen && (
        <div className="md:hidden mt-2 px-4 py-4 space-y-2 bg-white shadow-md rounded-lg mx-2">
          {[...navLinks, { name: "Contact", href: "/contact", highlight: "blue" }].map((link) => (
            <Link
              key={link.name}
              href={link.href}
              onClick={() => setIsMenuOpen(false)}
              className={`block px-4 py-2 text-lg rounded transition ${
                link.highlight === "orang"
                  ? "text-[var(--primary-orange)] font-semibold"
                  : link.highlight === "blue"
                  ? "text-[#4d53c4] font-semibold"
                  : "text-gray-800"
              } hover:bg-gray-100`}
            >
              {link.name}
